...
class FragmentChat4 : Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        ic_send.setOnClickListener {
            if (input_chat_message.text.isNotEmpty()) {
                mDatabase.child("messages/lecture4/").push().setValue(
                    Message(
    ...
    private fun displayChat() {
        if (isDark) {
            adapter = object : FirebaseListAdapter<Message>(activity, Message::class.java, R.layout.view_holder_message_dark, mDatabase.child("messages/lecture4/")) {
            ...
            adapter = object : FirebaseListAdapter<Message>(activity, Message::class.java, R.layout.view_holder_message, mDatabase.child("messages/lecture4/")) {
                override fun populateView(v: View?, model: Message?, position: Int) {
                    v!!.chat_message.text = model!!.message
                    v.chat_user.text = model.user
                    v.chat_time.text = DateFormat.format("dd-MM-yyyy HH:mm", model.time)
